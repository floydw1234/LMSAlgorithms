...
lmsFilter([1 0.2 -0.4 0.3 0.2 -0.1],25,0.01,10,300,30,100);
...
function y = lmsFilter(c,SNR,u,M,N,T,P)
    ...
    %       noisy signal here
            ...
            xBuff(j,1:M)*(xBuff(j,1:M)');

            allAdaptedFilters(j,1:M) = allAdaptedFilters(j,1:M) + (2*M*(u*conj(error(j)))*xBuff(j,1:M))/(xBuff(j,1:M)*xBuff(j,1:M)');
